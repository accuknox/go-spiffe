...
As you may noted, the three workloads create a [workloadapi.X509Source](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/workloadapi?tab=doc#X509Source) struct.
```go
...
The **server workload** uses the `X509Source` to create the [TLSServerConfig](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/spiffetls/tlsconfig?tab=doc#TLSServerConfig) for the HTTP server used:
```go
	server := &http.Server{
		Addr:      ":8080",
...
The **client workload** uses the `X509Source` to create the [TLSClientConfig](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/spiffetls/tlsconfig?tab=doc#TLSClientConfig) for the `Transport` of the HTTP client used:
```go
...
- The [Authorizer](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/spiffetls/tlsconfig?tab=doc#Authorizer) returned by [tlsconfig.AuthorizeID()](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/spiffetls/tlsconfig?tab=doc#AuthorizeID)

The **proxy workload** uses the `X509Source` to create the [TLSClientConfig](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/spiffetls/tlsconfig?tab=doc#TLSClientConfig) for the `Transport` of the HTTP reverse proxy used:
```go
...
- The trust bundle provided by the Workload API via the `X509Source`.
- The [Authorizer](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/spiffetls/tlsconfig?tab=doc#Authorizer) function returned by [tlsconfig.AuthorizeID()](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/spiffetls/tlsconfig?tab=doc#AuthorizeID)

The **proxy workload** also uses the `X509Source` to create the [TLSServerConfig](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/spiffetls/tlsconfig?tab=doc#TLSServerConfig) for the HTTP server used:
```go
	server := &http.Server{
		Addr:      ":8443",
		TLSConfig: tlsconfig.TLSServerConfig(x509Source),
...
On the scenario described we can see that only the client and the server workloads create a [workloadapi.JWTSource](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/workloadapi?tab=doc#JWTSource). This is because the proxy workload doesn't need to deal with JWTs since the server is the one in charge of authenticating the clients:
```go
...
The **client workload** uses the `JWTSource` to get a JWT-SVID by calling its [FetchJWTSVID](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/workloadapi?tab=doc#JWTSource.FetchJWTSVID) function:
```go
...
The **server workload** uses the `JWTSource` to authenticate the client by calling the [ParseAndValidate](https://pkg.go.dev/github.com/accuknox/go-spiffe/v2/svid/jwtsvid?tab=doc#ParseAndValidate) function:
```go
